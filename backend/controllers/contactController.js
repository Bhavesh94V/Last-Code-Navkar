...
// Helper function for appending data
// async function addToSheet(data) {
//     try {
//         await sheets.spreadsheets.values.append({
//             spreadsheetId: SPREADSHEET_ID,
//             range: "Sheet1!A1",  // Make sure your sheet is named "Sheet1"
...
//         // 1️⃣ Validate incoming request data
//         if (!name || !email || !phone || !message) {
//             return res.status(400).json({ success: false, message: 'Missing required fields' });
//         }
//         if (!/\S+@\S+\.\S+/.test(email)) {
...
//       <p>Submitted on: ${new Date().toLocaleString()}</p>
//     `;
//         await sendEmail({ to: process.env.EMAIL_USER, subject: `New Contact from ${name}`, html: emailHtml });
